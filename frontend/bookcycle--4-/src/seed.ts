...
if (Object.keys(dbOps.getAllUsers()).length === 0) {
  console.log('Seeding database...');
  Object.values(initialUsers).forEach(user => dbOps.upsertUser(user));
  initialBooks.forEach(book => dbOps.addBook(book));
  initialChats.forEach(chat => dbOps.addChat(chat));
  Object.entries(initialMessages).forEach(([chatId, msgs]) => {
    msgs.forEach(msg => dbOps.addMessage({ ...msg, chatId }));
  });
